...
TENANT_ID="xxxxxx"
WORKSPACE_ID="xxxxx"
DATASET_ID="xxxxxxxxx"
FILE_NAME="datasets/xxx.bim"
...
powerbi_dataset_update() {
  ...
  # update the dataset
  HTTP_STATUS_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH "${DATASET_URL}/datasets/${DATASET_ID}" -H "Authorization: Bearer ${ACCESS_TOKEN}" -d @${FILE_NAME}  -H "Content-Type: Application/json")
