...
```sql
-- create a stage
create or replace stage sfuser_ext_stage url='s3://snowflake-cookbook/ch2/r4/';
...
create or replace external table ext_card_data with location = @sfuser_ext_stage/csv
file_format = (type = csv) pattern = '.*headless[.]csv';

select * from ext_card_data; 

-- translate JSON data from external table into a relational format
select top 5 value:c3::float as card_sum, value:c2::string as period from ext_card_data;
