...
triggersNameList=$(echo "${triggersListResponse}" | jq  -r '.value[] | select(.properties.runtimeState == "Started") | .name')

for triggersName in ${triggersNameList}
do  
    apiURL="https://management.azure.com/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/${ADF_RESOURCE_GROUP_NAME}/providers/Microsoft.DataFactory/factories/${ADF_NAME}/triggers/${triggersName}/stop?api-version=2018-06-01"
    REQUEST_DATA='{}'
    ...
    echo "TRIGGER ${triggersName} is stopped and Status Code is ${http_status}"
done
