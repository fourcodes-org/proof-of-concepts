...
def create_excel_report(users, groups):
    ...
    # Populate the Excel sheet with group policy information
    for group_name in groups:
        policies = get_group_policy_information(iam_resource, group_name)
        if policies:
            group_arn = iam_resource.Group(group_name).arn
            for policy in policies:
                policy_type = policy["Type"]
                policy_name = policy["Name"]
                policy_arn = policy["Arn"]
                sheet.append([group_name, group_arn, policy_type, policy_name, policy_arn])
...
if __name__ == '__main__':
    try:
        ...
        all_groups = [group.name for group in iam_resource.groups.all()]

        if all_users or all_groups:
            print(f"Total IAM users found: {len(all_users)}")
            print(f"Total IAM groups found: {len(all_groups)}\n")
            create_excel_report(all_users, all_groups)
        else:
