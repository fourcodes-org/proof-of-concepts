...
allowed_agency_list_secret_name = "agency/information"
...
def cw_log_poster(key, action, state):
    return dict({'key': key, 'action': action, 'processed_state': state})
...
def lambda_handler(event, context):
    ...
    if (s3_processor.object_state() and pgp_email_receipt_state is not None):
        ...
        pgp_private_key = collect_specific_agent_details[0][agent_name]['agent_pgp_private_key']
        ...
        if (pgp_handle.check_file_extension(key)):
            ...
            decryption_status = pgp_handle.decrypt_file_with_gpg(pgp_private_key, decryption_source_key_download)
